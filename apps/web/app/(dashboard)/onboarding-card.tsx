...
export function OnboardingCard({ show, orgSlug }: Props) {
  ...
  return (
    <div className="rounded-xl border-2 border-emerald-200 dark:border-emerald-800/40 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/60 dark:to-green-950/30 p-6 relative overflow-hidden">
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-emerald-900 dark:text-emerald-100">Welcome to SalesAgent AI!</h2>
              <p className="text-sm text-emerald-700 dark:text-emerald-400">Follow these 3 steps to deploy your AI SDR.</p>
            </div>
          </div>
          <button onClick={() => setDismissed(true)} className="text-emerald-500 hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors text-sm">Dismiss</button>
        </div>
          ...
          {[
            ...
            <Link key={item.step} href={item.href} className="rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 p-5 hover:shadow-card-hover transition-all group">
              <div className="flex items-center gap-3 mb-3">
                <div className="size-7 rounded-full bg-emerald-600 text-white flex items-center justify-center text-sm font-bold shrink-0">{item.step}</div>
              ...
              <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 group-hover:underline">{item.cta} →</span>
            </Link>
          ))}
        </div>
      </div>
